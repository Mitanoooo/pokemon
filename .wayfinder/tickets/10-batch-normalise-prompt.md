...
- **Price is now fetched and used.** Step 1 returns each raw_name's `MIN`/`MAX` price, currency and sites alongside the name, and a new § 5f uses it to separate product forms: single packs read 4–12 EUR against 150–400 EUR for display boxes, and 26% of tracked raw_names are a bare "Booster"/"Boosteri" with no qualifier at all. It is documented as a **prior, not a rule** — scalped sets break the bands (a single Prismatic Evolutions pack at 20.95 EUR, a plain ETB at 149.00 EUR). Batch CSVs gain a trailing review-only `observed_price` column.
...
- **§ 5c rewritten to strip retailer noise first and search the whole catalog.** Five of the 25 calibration examples have their correct product outside the `difflib` top five, so ranking by string similarity and stopping at the top few is explicitly ruled out.
